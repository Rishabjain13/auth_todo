document.addEventListener("DOMContentLoaded",async () => {
    ...
    function initActivityFeed() {
        ...
        if (window.activitySocket) return; // prevent duplicates

        window.activitySocket = new WebSocket(
            `ws://localhost:8000/ws/activity?token=${token}`
        );

        window.activitySocket.onmessage = (event) => {
            const data = JSON.parse(event.data);
            const feed = document.getElementById("activityFeed");
            if (!feed) return;

            const key = `${data.actor_email}-${data.action}-${data.title}`;
            if (feed.dataset.last === key) return;
            feed.dataset.last = key;

            const p = document.createElement("p");
            p.innerHTML = `
                <b>${data.actor_email}</b>
                ${data.action}
                <i>${data.title ?? ""}</i>
            `;
            feed.prepend(p);
